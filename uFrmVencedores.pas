...
begin
  diag := TFancyDialog.Create(FrmVencedores);

  DmPrincipal.qryVencedores.Active := False;
  DmPrincipal.qryVencedores.Active := True;
  Atualizar := False;

  if DmPrincipal.qryVencedores.FieldByName('DATE').AsDateTime <> null then
  begin
  ...
  begin
    DmPrincipal.qryVencedores.Edit;
    DmPrincipal.qryVencedores.FieldByName('DATE').AsDateTime := date;
    DmPrincipal.qryVencedores.Post;
  end;
